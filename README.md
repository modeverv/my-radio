...
ローカル環境（macOS）で動作する、自動ラジオ局システムです。NHKニュースから最新ヘッドラインを取得し、YouTube Music の日本チャートから選曲。ローカルLLMでMC台本を生成し、VOICEVOXで読み上げながらブラウザ（Selenium）で楽曲を再生します。
＜開発中＞
初っ端が不安定
...
- **Context:** NHK News RSS + Time-based Mood (`context_builder.py`)
- **Director:** LM Studio (Local LLM - MC Script & Song Selection Logic) (`llm_director.py`)
- **Player:** YouTube Music via Selenium/Chrome (`ytm_player.py`)
- **TTS:** VOICEVOX (Zundamon) (`tts.py`)
...
make run
```
1セグメント（MCトーク + 楽曲1曲）のサイクルで無限ループ実行されます。
...
- `llm_director.py`: LLM を用いた要約・台本作成。
